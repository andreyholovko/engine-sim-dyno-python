...
Uses the `ls2_loop` fixture (tests/conftest.py).
"""


def test_peak_torque_matches_published_figure(ls2_loop):
    readings = ls2_loop.run_power_pull()
...
def test_peak_power_matches_published_figure(ls2_loop):
    readings = ls2_loop.run_power_pull()
...
def test_torque_rises_from_idle_to_peak_on_ve_alone(ls2_loop):
...
def test_naturally_aspirated_never_builds_boost(ls2_loop):
    readings = ls2_loop.run_power_pull()
    assert all(r.boost_bar < 1e-6 for r in readings)


def test_power_pull_terminates_at_redline(ls2_loop):
    readings = ls2_loop.run_power_pull()
    assert readings[-1].rpm >= ls2_loop.ecu.rev_limiter_threshold_rpm
